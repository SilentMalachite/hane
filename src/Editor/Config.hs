            ...
            if not exists
                then return $ Left $ ConfigFileNotFound file
                    ...
                    case parseConfigText content of
                        ...
                        Right base -> do
                            let loc = maybe (cfgLocale base) T.pack envLoc
...
parseConfigText txt = do
    ...
    let ls0 = map (T.takeWhile (/= '#')) (T.lines txt)
        ...
        fos' =
            fromMaybe
                (cfgFormatOnSave defaultConfig)
                (lookupKey "formatonsave" >>= parseBool)
        kmText =
            fromMaybe
                (T.toLower $ T.pack $ show $ cfgKeymap defaultConfig)
                (lookupKey "keymap")
...
validateConfig config = do
    _ <-
        unless (validateTheme (cfgTheme config)) $
            Left $
                InvalidThemeError (cfgTheme config)
    _ <-
        unless (validateLocale (cfgLocale config)) $
            Left $
                InvalidLocaleError (cfgLocale config)
    Right config
  where
