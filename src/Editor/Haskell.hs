...
formatWith exe args input = do
  ...
  case mx of
    ...
    Just _  -> do
      let p = setStdin (byteStringInput (BL.fromStrict (TE.encodeUtf8 input)))
...
quickCheck path = do
  ...
  case mGhc of
    Just _ -> do
      let p = setStdout byteStringOutput
            $ setStderr byteStringOutput
...
startRepl = do
  ...
  case mCabal of
    ...
    Nothing -> do
      mGhci <- findExecutable "ghci"
      case mGhci of
        Just _ -> runProcess_ ( setStdin inherit
                              $ setStdout inherit
                              $ setStderr inherit
                              $ proc "ghci" [] )
        Nothing -> pure ()
